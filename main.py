...
# --- AYARLAR ---
map_filename = "rota_haritasi.html"
OSRM_BASE_URL = "http://router.project-osrm.org"
SERVICE_TIME = 10  # Her durakta harcanan süre (Dakika)
...
def optimize(request: OptimizationRequest):
    locations = [
        {"lat": j.lat, "lon": j.lon, "id": j.id, "demand": j.demand, "time_start": j.time_start, "time_end": j.time_end}
    ...
    routing.AddDimension(transit_callback_index, 0, 99999999, True, dimension_name)  # Limiti sonsuza açtık
    distance_dimension = routing.GetDimensionOrDie(dimension_name)
    distance_dimension.SetGlobalSpanCostCoefficient(1)

    # --- ZAMAN BOYUTU (SINIRSIZ ESNEKLİK) ---
    routing.AddDimension(
        time_callback_index,
        99999,  # Slack (Bekleme payı) -> Sınırsız
        99999,  # Kapasite (Maksimum rota süresi) -> Sınırsız
        False,
        'Time'
    )
    ...
    for vehicle_id in range(request.vehicle_count):
        ...
        # Sadece araçların çıkış saatini sabah 08:00'a sabitliyoruz. Dönüş saati sonsuz serbest.
        time_dimension.CumulVar(start_index).SetValue(480)

    # 2. MÜŞTERİ ZAMAN PENCERELERİ
    for i in range(1, len(locations)):
        ...
        # SİHİRLİ DOKUNUŞ: Eğer Sheet'te saat boş bırakıldıysa hiçbir kısıt ekleme! Pas geç.
        if start_t == 480 and end_t == 1080:
            continue

        # Eğer saat girilmişse:
        # Erken gelirse kapıda beklesin (Minimum limit)
        time_dimension.CumulVar(index).SetMin(start_t)

        # Geç kalırsa ÇÖKMESİN, sadece motora ceza yaz (Yumuşak limit - Soft Bound)
        time_dimension.SetCumulVarSoftUpperBound(index, end_t, 100)

    # --- ARAMA PARAMETRELERİ ---
    search_parameters = pywrapcp.DefaultRoutingSearchParameters()
    # Hata oranını en aza indiren en istikrarlı algoritma: PATH_CHEAPEST_ARC
    ...
    if solution:
        ...
        for vehicle_id in range(request.vehicle_count):
            ...
            if len(path_stops) <= 1:
                routes_json.append(
                    {"vehicle_id": vehicle_id + 1, "path": [], "geometry": [], "total_km": 0, "total_load": 0})
                continue
            ...
            path_stops.append(
                {"lat": depot.lat, "lon": depot.lon, "id": depot.id, "demand": 0, "arrival_time": arrival_time_str})
        ...
        create_map(request.jobs, result_data)
        return result_data
    else:
        raise HTTPException(status_code=400,
                            detail="Rota bulunamadı! Lütfen verilerinizi (koordinat vb.) kontrol edin.")


@app.get("/map", response_class=HTMLResponse)
def get_map():
    if os.path.exists(map_filename): return FileResponse(map_filename)
    return "<h1>Rota yok.</h1>"


def create_map(jobs, result_json):
    if not result_json.get("routes"): return
    ...
    for i, route in enumerate(result_json["routes"]):
        ...
        for stop in path:
            ...
            if orj_id == 0:
                if order == 1:
                    folium.Marker([lat, lon], popup="DEPO (Çıkış: 08:00)",
                ...
                folium.Marker([lat, lon], tooltip=f"Durak {display_num}: {demand} Kg | Saat: {arrival}",
                              opacity=0).add_to(m)
    m.save(map_filename)